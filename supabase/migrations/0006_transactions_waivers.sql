...
create table if not exists public.transactions (
  ...
  type           text not null check (type in
                   ('add','drop','waiver_add','waiver_drop','waiver_failed',
...
create table if not exists public.waiver_claims (
  ...
  add_player_id     text references public.nfl_players(id) on delete cascade,
  add_dst_abbr      text references public.nfl_teams(abbr) on delete cascade,
  -- optional corresponding drop, so the roster stays legal
  drop_player_id    text references public.nfl_players(id) on delete set null,
  drop_dst_abbr     text references public.nfl_teams(abbr) on delete set null,
  ...
  check ((add_player_id is null) <> (add_dst_abbr is null))
);
...
create unique index if not exists waiver_claims_one_per_player
  on public.waiver_claims(team_id, add_player_id)
  where status = 'pending' and add_player_id is not null;
...
create unique index if not exists waiver_holds_player_unique
  on public.waiver_holds(league_id, player_id) where player_id is not null;
create unique index if not exists waiver_holds_dst_unique
  on public.waiver_holds(league_id, dst_team_abbr) where dst_team_abbr is not null;
...
-- One row per asset moving in a trade.
create table if not exists public.trade_items (
  ...
  check (
    ...
    + (dst_team_abbr is not null)::int
    + (faab_amount is not null)::int = 1
  )
);
