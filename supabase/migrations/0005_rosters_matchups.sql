...
create table if not exists public.roster_players (
  ...
  -- exactly one of these is set: a real player, or a team D/ST
...
create unique index if not exists roster_players_one_owner
  on public.roster_players(league_id, player_id)
  where dropped_at is null and player_id is not null;
create unique index if not exists roster_players_one_owner_dst
  on public.roster_players(league_id, dst_team_abbr)
  where dropped_at is null and dst_team_abbr is not null;

-- The weekly starting lineup. One row per rostered player per week.
...
create unique index if not exists lineup_player_unique
  on public.lineup_entries(team_id, season, week, player_id)
  where player_id is not null;
create unique index if not exists lineup_dst_unique
  on public.lineup_entries(team_id, season, week, dst_team_abbr)
  where dst_team_abbr is not null;
...
-- simple group-by gives the record, points for and points against.
create or replace view public.standings as
with sides as (
  select league_id, season, week, home_team_id as team_id,
         away_team_id as opponent_id, home_score as pf, away_score as pa,
         status, is_playoff
  from public.matchups
  union all
  select league_id, season, week, away_team_id as team_id,
         home_team_id as opponent_id, away_score as pf, home_score as pa,
         status, is_playoff
  from public.matchups
...
select
  ...
  t.name                                                        as team_name,
  t.owner_id,
  coalesce(count(s.*) filter (where s.status = 'final'), 0)      as games_played,
