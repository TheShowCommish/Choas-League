...
|----|---|------|-------|--------|---------------------|
| T-007 | P0 | A 0-point position override must mean zero (0030:113 filters points<>0 so WR override 0 falls back to base). Projections must match. | engineer | READY | Base tackle 5 + WR override 0 → WR 0, QB 5; projections agree; regression test |
| T-008 | P0 | Real, independent losers bracket: own rounds/teams/byes/names, advances round by round, entrants (eliminated / non-playoff / both), mode (consolation vs toilet bowl), all per-league settings with no assumed format; UI preview start weeks match DB (playoff-rounds.tsx:104 vs 0034:765). | engineer→designer | READY | All config fields honored; both modes tested; preview == DB; league isn't marked complete while any playoff/losers game is open (else it can't auto-finalize) |
